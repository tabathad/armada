...
func TestQueueCandidateGangIterator(t *testing.T) {
	tests := map[string]struct {
		...
		"all jobs schedulable": {
			Reqs:            scheduler.testNSmallCpuJob("A", 0, 3),
			ExpectedIndices: []int{0, 1, 2},
		},
		"minimum job size below limit": {
			Reqs: append(scheduler.testNSmallCpuJob("A", 0, 3), scheduler.testNLargeCpuJob("A", 0, 2)...),
			SchedulingConstraints: SchedulingConstraints{
		...
		"lookback limit hit": {
			Reqs: scheduler.testNSmallCpuJob("A", 0, 10),
			SchedulingConstraints: SchedulingConstraints{
		...
		"MaxConsecutiveUnschedulableJobs": {
			Reqs: append(append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNGpuJob("A", 0, 3)...), scheduler.testNSmallCpuJob("A", 0, 1)...),
			SchedulingConstraints: SchedulingConstraints{
				TotalResources: schedulerobjects.ResourceList{
					Resources: map[string]resource.Quantity{
						"cpu": resource.MustParse("32"),
		...
		"MaximalCumulativeResourceFractionPerQueueAndPriority": {
			Reqs: append(append(scheduler.testNSmallCpuJob("A", 9, 11), scheduler.testNSmallCpuJob("A", 7, 11)...), scheduler.testNSmallCpuJob("A", 3, 11)...),
			SchedulingConstraints: SchedulingConstraints{
				TotalResources: schedulerobjects.ResourceList{
					Resources: map[string]resource.Quantity{
						"cpu": resource.MustParse("32"),
			...
			LeaseJobs:       true,
			ExpectedIndices: append(append(scheduler.intRange(0, 9), scheduler.intRange(11, 20)...), scheduler.intRange(22, 31)...),
		},
		"MaximalCumulativeResourceFractionPerQueueAndPriority with initial usage": {
			...
			LeaseJobs:       true,
			ExpectedIndices: append(append(scheduler.intRange(0, 6), scheduler.intRange(11, 18)...), scheduler.intRange(22, 30)...),
		},
	}
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			queuedGangIterator := NewQueuedGangIterator(
				...
				scheduler.testGangIdAnnotation,
				scheduler.testGangCardinalityAnnotation,
			)
...
func TestSchedule(t *testing.T) {
	tests := map[string]struct {
		...
		"one queue one job": {
			SchedulingConfig: scheduler.testSchedulingConfig(),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 1),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"one queue several jobs": {
			...
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 32),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
			ExpectedIndicesByQueue: map[string][]int{
				"A": scheduler.intRange(0, 31),
			},
			ExpectedIndicesByNodeIndexAndQueue: map[int]map[string][]int{
				0: {"A": scheduler.intRange(0, 31)},
			},
		},
		"one queue some jobs do not fit": {
			...
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 33),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"one queue with jobs of varying size": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 0, 32), scheduler.testNLargeCpuJob("A", 0, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
			ExpectedIndicesByQueue: map[string][]int{
				"A": scheduler.intRange(0, 32),
			},
		},
		"preempt lower-priority jobs": {
			...
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 1, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"no preemption of higher-priority jobs": {
			...
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNLargeCpuJob("A", 1, 1), scheduler.testNSmallCpuJob("A", 0, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"unschedulable jobs do not block schedulable jobs": {
			...
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 0, 10)...), scheduler.testNSmallCpuJob("A", 0, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"max jobs to schedule limit": {
			SchedulingConfig: scheduler.withMaxJobsToScheduleConfig(2, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 5),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"round limits": {
			SchedulingConfig: scheduler.withRoundLimitsConfig(map[string]float64{"cpu": 2.0 / 32.0}, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 5),
			},
		...
		"round per-queue limits": {
			SchedulingConfig: scheduler.withPerQueueRoundLimitsConfig(map[string]float64{"cpu": 2.0 / 32.0}, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 5),
				"B": scheduler.testNSmallCpuJob("A", 0, 5),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
				"B": 1,
		...
		"overall per-queue limits": {
			SchedulingConfig: scheduler.withPerQueueLimitsConfig(map[string]float64{"cpu": 2.0 / 32.0}, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 5),
				"B": scheduler.testNSmallCpuJob("A", 0, 5),
		...
		"overall per-queue limits with large memory amount": {
			SchedulingConfig: scheduler.withPerQueueLimitsConfig(
				map[string]float64{
					"cpu":    2.0 / 162975640.0,
					"memory": 0.1,
				},
				scheduler.testSchedulingConfig()),
			Nodes: scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 5),
				"B": scheduler.testNSmallCpuJob("A", 0, 5),
		...
		"per priority per-queue limits": {
			SchedulingConfig: scheduler.withPerPriorityLimitsConfig(
			...
			Nodes: scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 3, 5), scheduler.testNSmallCpuJob("A", 0, 5)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"per priority per queue limits equal limits": {
			...
			Nodes: scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 0, 5), scheduler.testNSmallCpuJob("A", 0, 5)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"limit hit at higher priority doesn't block jobs at lower priority": {
			...
			Nodes: scheduler.testNCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 1, 1), scheduler.testNSmallCpuJob("A", 0, 5)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"fairness two queues": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 32),
				"B": scheduler.testNSmallCpuJob("A", 0, 32),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
				"B": 1,
		...
		"fairness three queues": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 32),
				"B": scheduler.testNSmallCpuJob("A", 0, 32),
				"C": scheduler.testNSmallCpuJob("A", 0, 32),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
				"B": 1,
		...
		"weighted fairness two queues": {
			SchedulingConfig: scheduler.testSchedulingConfig(),
			Nodes:            scheduler.testNCpuNode(3, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 96),
				"B": scheduler.testNSmallCpuJob("A", 0, 96),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
				"B": 2,
		...
		"weighted fairness three queues": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 96),
				"B": scheduler.testNSmallCpuJob("A", 0, 96),
				"C": scheduler.testNSmallCpuJob("A", 0, 96),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
				"B": 2,
		...
		"Node with no available capacity": {
			SchedulingConfig: scheduler.testSchedulingConfig(),
			Nodes: scheduler.withUsedResourcesNodes(
				0,
		...
		"Node with some available capacity": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNSmallCpuJob("A", 0, 2),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"preempt used resources of lower-priority jobs": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.testNLargeCpuJob("A", 1, 1),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"respect taints": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 0, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"minimum job size gpu": {
			SchedulingConfig: scheduler.testSchedulingConfig(),
			Nodes:            scheduler.testNGpuNode(2, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 0, 1)...), scheduler.testNGpuJob("A", 0, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"Node selector": {
			...
			Nodes: append(
				scheduler.testNCpuNode(1, scheduler.testPriorities),
				scheduler.withLabelsNodes(map[string]string{"foo": "foo"}, scheduler.testNCpuNode(1, scheduler.testPriorities))...,
			),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.withNodeSelectorPodReqs(map[string]string{"foo": "foo"}, scheduler.testNLargeCpuJob("A", 0, 2)),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"taints and tolerations (indexed)": {
			SchedulingConfig: scheduler.withIndexedTaintsConfig([]string{"largeJobsOnly"}, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNTaintedCpuNode(1, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 0, 1)...),
			},
		...
		"Node selector (indexed)": {
			SchedulingConfig: scheduler.withIndexedNodeLabelsConfig([]string{"foo"}, scheduler.testSchedulingConfig()),
			Nodes: append(
				scheduler.testNCpuNode(1, scheduler.testPriorities),
				scheduler.withLabelsNodes(map[string]string{"foo": "foo"}, scheduler.testNCpuNode(1, scheduler.testPriorities))...,
			),
		...
		"QueueLeaseBatchSize Respected": {
			SchedulingConfig: scheduler.withQueueLeaseBatchSizeConfig(3, scheduler.testSchedulingConfig()), // should quit after 3 unschedulable jobs
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),                          // 32 cores
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(append(scheduler.testNSmallCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 0, 3)...), scheduler.testNSmallCpuJob("A", 0, 1)...),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"gang scheduling success": {
			...
			Nodes:            scheduler.testNCpuNode(2, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.withGangAnnotationsPodReqs(scheduler.testNLargeCpuJob("A", 0, 2)),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"gang scheduling failure": {
			...
			Nodes:            scheduler.testNCpuNode(2, scheduler.testPriorities),
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": scheduler.withGangAnnotationsPodReqs(scheduler.testNLargeCpuJob("A", 0, 3)),
			},
			PriorityFactorByQueue: map[string]float64{
				"A": 1,
			},
		...
		"gang aggregated resource accounting": {
			...
			ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
				"A": append(append(
					scheduler.withAnnotationsPodReqs(map[string]string{scheduler.testGangIdAnnotation: "my-gang", scheduler.testGangCardinalityAnnotation: "2"}, scheduler.testNSmallCpuJob("A", 0, 1)),
	...
	tests := map[string]struct {
		...
		"balancing three queues": {
			...
			Rounds: []ReschedulingRound{
				...
				},
				{
					...
					ExpectedScheduledIndices: map[string][]int{
						"B": scheduler.intRange(0, 15),
					},
					ExpectedPreemptedIndices: map[string]map[int][]int{
						"A": {
							0: scheduler.intRange(16, 31),
						},
					},
				},
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"C": scheduler.testNSmallCpuJob("A", 0, 10),
					},
					ExpectedScheduledIndices: map[string][]int{
						"C": scheduler.intRange(0, 9),
					},
					ExpectedPreemptedIndices: map[string]map[int][]int{
						"A": {
							0: scheduler.intRange(11, 15),
						},
						"B": {
							1: scheduler.intRange(11, 15),
						},
					},
				},
			},
		...
		"balancing two queues weighted": {
			...
			Rounds: []ReschedulingRound{
				...
				},
				{
					...
					ExpectedScheduledIndices: map[string][]int{
						"B": scheduler.intRange(0, 20),
					},
					ExpectedPreemptedIndices: map[string]map[int][]int{
						"A": {
							0: scheduler.intRange(11, 31),
						},
					},
				},
			},
		...
		"reschedule onto same node": {
			...
			Rounds: []ReschedulingRound{
				...
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"B": scheduler.testNSmallCpuJob("B", 0, 32),
					},
					ExpectedScheduledIndices: map[string][]int{
						"B": scheduler.intRange(0, 31),
					},
				},
			},
			PriorityFactorByQueue: map[string]float64{
		...
		"rescheduled jobs don't count towards maxJobsToSchedule": {
			SchedulingConfig: scheduler.withMaxJobsToScheduleConfig(5, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			Rounds: []ReschedulingRound{
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"A": scheduler.testNSmallCpuJob("A", 0, 10),
					},
					ExpectedScheduledIndices: map[string][]int{
						"A": scheduler.intRange(0, 4),
					},
				},
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
		...
		"rescheduled jobs don't count towards maxLookbackPerQueue": {
			SchedulingConfig: scheduler.withMaxLookbackPerQueueConfig(5, scheduler.testSchedulingConfig()),
			Nodes:            scheduler.testNCpuNode(1, scheduler.testPriorities),
			Rounds: []ReschedulingRound{
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
		...
		"rescheduled jobs don't count towards MaximalResourceFractionToSchedulePerQueue": {
			SchedulingConfig: scheduler.withPerQueueRoundLimitsConfig(
				map[string]float64{
					"cpu": 5.0 / 32.0,
				},
				scheduler.testSchedulingConfig(),
		...
		"rescheduled jobs don't count towards MaximalClusterFractionToSchedule": {
			SchedulingConfig: scheduler.withRoundLimitsConfig(
				map[string]float64{
					"cpu": 5.0 / 32.0,
				},
				scheduler.testSchedulingConfig(),
		...
		"priority class preemption two classes": {
			...
			Rounds: []ReschedulingRound{
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"A": scheduler.testNLargeCpuJob("A", 0, 1),
					},
					ExpectedScheduledIndices: map[string][]int{
						"A": scheduler.intRange(0, 0),
					},
				},
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
		...
		"priority class preemption cross-queue": {
			...
			Rounds: []ReschedulingRound{
				...
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"B": scheduler.testNLargeCpuJob("B", 1, 1),
					},
					ExpectedScheduledIndices: map[string][]int{
						"B": scheduler.intRange(0, 0),
					},
					ExpectedPreemptedIndices: map[string]map[int][]int{
						"A": {
							0: scheduler.intRange(0, 0),
		...
		"priority class preemption not scheduled": {
			...
			Rounds: []ReschedulingRound{
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"A": append(scheduler.testNLargeCpuJob("A", 0, 1), scheduler.testNLargeCpuJob("A", 1, 1)...),
					},
					ExpectedScheduledIndices: map[string][]int{
						"A": scheduler.intRange(1, 1),
					},
				},
			},
			PriorityFactorByQueue: map[string]float64{
			...
			Rounds: []ReschedulingRound{
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"A": append(append(
							scheduler.testNSmallCpuJob("A", 0, 10),
							scheduler.testNSmallCpuJob("A", 1, 10)...),
							scheduler.testNSmallCpuJob("A", 2, 10)...,
						),
					},
					ExpectedScheduledIndices: map[string][]int{
						"A": scheduler.intRange(0, 29),
					},
				},
				{
					ReqsByQueue: map[string][]*schedulerobjects.PodRequirements{
						"A": scheduler.testNSmallCpuJob("A", 3, 24),
					},
					ExpectedScheduledIndices: map[string][]int{
						"A": scheduler.intRange(0, 23),
					},
					ExpectedPreemptedIndices: map[string]map[int][]int{
						"A": {
							0: append(scheduler.intRange(0, 19), scheduler.intRange(28, 29)...),
						},
					},
				},
			},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for i, round := range tc.Rounds {
				...
				prioritiesByName := configuration.PriorityByPriorityClassName(scheduler.testPriorityClasses)
...
func BenchmarkReschedule(b *testing.B) {
	tests := map[string]struct {
		...
		"1 node 1 queue 32 jobs": {
			SchedulingConfig:                      scheduler.testSchedulingConfig(),
			Nodes:                                 scheduler.testNCpuNode(1, scheduler.testPriorities),
			PodReqFunc:                            scheduler.testNSmallCpuJob,
			NumQueues:                             1,
			NumJobsPerQueue:                       32,
			MinPriorityFactor:                     1,
			MaxPriorityFactor:                     1,
		...
		"10 nodes 1 queue 320 jobs": {
			SchedulingConfig:                      scheduler.testSchedulingConfig(),
			Nodes:                                 scheduler.testNCpuNode(10, scheduler.testPriorities),
			PodReqFunc:                            scheduler.testNSmallCpuJob,
			NumQueues:                             1,
			NumJobsPerQueue:                       320,
			MinPriorityFactor:                     1,
		...
		"100 nodes 1 queue 3200 jobs": {
			SchedulingConfig:                      scheduler.testSchedulingConfig(),
			Nodes:                                 scheduler.testNCpuNode(100, scheduler.testPriorities),
			PodReqFunc:                            scheduler.testNSmallCpuJob,
			NumQueues:                             1,
		...
		"1000 nodes 1 queue 32000 jobs": {
			SchedulingConfig:                      scheduler.testSchedulingConfig(),
			Nodes:                                 scheduler.testNCpuNode(1000, scheduler.testPriorities),
			PodReqFunc:                            scheduler.testNSmallCpuJob,
			NumQueues:                             1,
		...
		"mixed": {
			SchedulingConfig:                      scheduler.testSchedulingConfig(),
			Nodes:                                 scheduler.testNCpuNode(500, scheduler.testPriorities),
			PodReqFunc:                            scheduler.testNSmallCpuJob,
			NumQueues:                             100,
			NumJobsPerQueue:                       256,
			MinPriorityFactor:                     1,
...
func TestEvictOversubscribed(t *testing.T) {
	...
	var err error
	jobs := append(
		legacySchedulerJobsFromPodReqs("A", "priority-0", scheduler.testNSmallCpuJob("A", 0, 20)),
		legacySchedulerJobsFromPodReqs("A", "priority-1", scheduler.testNSmallCpuJob("A", 1, 20))...,
	)
	reqs := PodRequirementsFromLegacySchedulerJobs(jobs, scheduler.testPriorityClasses)
	...
	jobRepo := NewInMemoryJobRepository(scheduler.testPriorityClasses)
...
func legacySchedulerJobsFromPodReqs(queue, priorityClassName string, reqs []*schedulerobjects.PodRequirements) []LegacySchedulerJob {
	...
	for priorityClassName, priorityClass := range scheduler.testPriorityClasses {
	...
	for i, req := range reqs {
		...
		if jobId := job.Annotations[config.JobIdAnnotation]; jobId != "" {
			job.Id = jobId
		}
		rv[i] = job
	}
